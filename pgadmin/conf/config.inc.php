...
{
    $configFile = dirname(__FILE__) . "/../../website/lst/configure.ini";
...
function IsLocalEnvironment()
{
    ...
    //fallback server
    if (GetFromConfig("ForceNotLocal", "env") == 1) {
        return false;
    }
...
if (IsLocalEnvironment() || GetFromConfig("ForceNotLocal", "env") == 1) {
    /**
...
// Time in seconds. If set to 0, refreshing data using ajax will be disabled (locks and activity pages)
    ...
    foreach ($service_blob as $service_provider => $service_list) {
        ...
        if ($service_provider === 'PostgreSQL' || $service_provider === 'osb-postgresql') {
    ...
    for ($i = 0; $i < count($pgsql_services); $i++) {
        ...
        if (count($db) == 6) {
            ...
            // Specify the path to the database dump utilities for this server.
            // You can set these to '' if no dumper is available.
            //$conf['servers'][$i]['pg_dump_path'] = exec("locate '*/pg_dump'");
            //$conf['servers'][$i]['pg_dumpall_path'] = exec("locate '*/pg_dumpall'");
            $conf['servers'][$i]['pg_dump_path'] = '/home/vcap/deps/0/apt/usr/lib/postgresql/10/bin/pg_dump';
            $conf['servers'][$i]['pg_dumpall_path'] = '/home/vcap/deps/0/apt/usr/lib/postgresql/10/bin/pg_dumpall';
        }
    }
