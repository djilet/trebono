        ...
        if ($type == 'table') { // cluster one or more table
            if (is_array($_REQUEST['table'])) {
                ...
                foreach ($_REQUEST['table'] as $o) {
                    ...
                    if ($status == 0) {
                        $msg .= sprintf('%s: %s<br />', htmlentities($o, ENT_QUOTES, 'UTF-8'),
                            $lang['strclusteredgood']);
                    } else {
                        doDefault($type, sprintf('%s%s: %s<br />', $msg, htmlentities($o, ENT_QUOTES, 'UTF-8'),
        ...
        if (($type == 'table') && is_array($_REQUEST['table'])) {
            ...
            foreach ($_REQUEST['table'] as $o) {
                ...
                if ($status == 0) {
                    ...
                    doDefault($type,
                        sprintf('%s%s: %s<br />', $msg, htmlentities($o, ENT_QUOTES, 'UTF-8'), $lang['strreindexbad']));
                    return;
                }
            }
            // Everything went fine, back to the Default page....
        ...
        if (($type == 'table') && is_array($_REQUEST['table'])) {
            ...
            foreach ($_REQUEST['table'] as $o) {
                ...
                if ($status == 0) {
                    ...
                    doDefault($type,
                        sprintf('%s%s: %s<br />', $msg, htmlentities($o, ENT_QUOTES, 'UTF-8'), $lang['stranalyzebad']));
                    return;
                }
            }
            // Everything went fine, back to the Default page....
        ...
        if (is_array($_REQUEST['table'])) {
            ...
            foreach ($_REQUEST['table'] as $t) {
                ...
                if ($status == 0) {
                    ...
                    doDefault($type,
                        sprintf('%s%s: %s<br />', $msg, htmlentities($t, ENT_QUOTES, 'UTF-8'), $lang['strvacuumbad']));
                    return;
                }
            }
            // Everything went fine, back to the Default page....
            ...
            $status = $data->vacuumDB($_REQUEST['table'], isset($_REQUEST['vacuum_analyze']),
                isset($_REQUEST['vacuum_full']), isset($_REQUEST['vacuum_freeze']));
            if ($status == 0) {
                $_reload_browser = true;
...
function doEditAutovacuum($type, $confirm, $msg = '')
{
    ...
    if ($confirm) {
        ...
        $status = $data->saveAutovacuum($_REQUEST['table'], $_POST['autovacuum_enabled'],
            $_POST['autovacuum_vacuum_threshold'],
            $_POST['autovacuum_vacuum_scale_factor'], $_POST['autovacuum_analyze_threshold'],
...
function doDropAutovacuum($type, $confirm)
{
    ...
    if ($confirm) {
        ...
        echo $misc->form;
        echo "<input type=\"hidden\" name=\"table\" value=\"", htmlspecialchars($_REQUEST['table']), "\" />\n";
        echo "<input type=\"hidden\" name=\"rel\" value=\"", htmlspecialchars(serialize(array(
            $_REQUEST['schema'],
            $_REQUEST['table']
        ))), "\" />\n";
    ...
    if ($data->hasAutovacuum()) {
        ...
        $columns = array(
            ...
            'autovacuum_vacuum_threshold' => array(
                ...
                'field' => callback('enlight',
                    array('autovacuum_vacuum_threshold', $defaults['autovacuum_vacuum_threshold'])),
                'type' => 'verbatim'
            ),
            'autovacuum_vacuum_scale_factor' => array(
                'title' => $lang['strvacuumscalefactor'],
                'field' => callback('enlight',
                    array('autovacuum_vacuum_scale_factor', $defaults['autovacuum_vacuum_scale_factor'])),
                'type' => 'verbatim'
            ),
            'autovacuum_analyze_threshold' => array(
                'title' => $lang['stranalybasethreshold'],
                'field' => callback('enlight',
                    array('autovacuum_analyze_threshold', $defaults['autovacuum_analyze_threshold'])),
                'type' => 'verbatim'
            ),
            'autovacuum_analyze_scale_factor' => array(
                'title' => $lang['stranalyzescalefactor'],
                'field' => callback('enlight',
                    array('autovacuum_analyze_scale_factor', $defaults['autovacuum_analyze_scale_factor'])),
                'type' => 'verbatim'
            ),
            'autovacuum_vacuum_cost_delay' => array(
                'title' => $lang['strvacuumcostdelay'],
                'field' => concat(callback('enlight',
                    array('autovacuum_vacuum_cost_delay', $defaults['autovacuum_vacuum_cost_delay'])), 'ms'),
                'type' => 'verbatim'
            ),
            'autovacuum_vacuum_cost_limit' => array(
                'title' => $lang['strvacuumcostlimit'],
                'field' => callback('enlight',
                    array('autovacuum_vacuum_cost_limit', $defaults['autovacuum_vacuum_cost_limit'])),
                'type' => 'verbatim'
            ),
        );
