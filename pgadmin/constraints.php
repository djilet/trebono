    ...
    switch ($stage) {
        ...
        case 3:
            ...
            if (!isset($_POST['IndexColumnList']) || !is_array($_POST['IndexColumnList'])
                ...
                $status = $data->addForeignKey($_POST['table'], $_POST['target']['schemaname'],
                    $_POST['target']['tablename'],
                    unserialize($_POST['SourceColumnList']), $_POST['IndexColumnList'], $_POST['upd_action'],
                    $_POST['del_action'],
...
function doDrop($confirm)
{
    ...
    if ($confirm) {
        ...
        $status = $data->dropConstraint($_POST['constraint'], $_POST['table'], $_POST['type'],
            isset($_POST['cascade']));
        if ($status == 0) {
...
function doDefault($msg = '')
{
    ...
    function cnPre(&$rowdata)
    {
        ...
        if (is_null($rowdata->fields['consrc'])) {
            ...
            $rowdata->fields['+definition'] = ($rowdata->fields['contype'] == 'u' ? "UNIQUE (" : "PRIMARY KEY (") . join(',',
                    $atts) . ')';
        } else {
