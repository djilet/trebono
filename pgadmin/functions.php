        ...
        if ($fnlang == 'c') {
            ...
            if ($fnlang == 'internal') {
                ...
                if (isset($data->langmap[$funcdata->fields['prolanguage']])) {
                    $temp = syntax_highlight(htmlspecialchars($funcdata->fields['prosrc']),
                        $data->langmap[$funcdata->fields['prolanguage']]);
                    $tag = 'prenoescape';
                } else {
                    $temp = $funcdata->fields['prosrc'];
                    $tag = 'pre';
                }
                echo "<tr><td class=\"data1\" colspan=\"4\">", $misc->printVal($temp, $tag,
                    array('lineno' => true, 'class' => 'data1')), "</td></tr>\n";
            }
        }
        ...
        if (is_array($_POST['function_oid'])) {
            ...
            if ($status == 0) {
                foreach ($_POST['function_oid'] as $k => $s) {
                    ...
                    if ($status == 0) {
                        $msg .= sprintf('%s: %s<br />', htmlentities($_POST['function'][$k], ENT_QUOTES, 'UTF-8'),
                            $lang['strfunctiondropped']);
                    } else {
                        $data->endTransaction();
                        doDefault(sprintf('%s%s: %s<br />', $msg,
                            htmlentities($_POST['function'][$k], ENT_QUOTES, 'UTF-8'), $lang['strfunctiondroppedbad']));
                        return;
                    }
                }
            }
...
function doSaveCreate()
{
    ...
    if ($_POST['formFunction'] == '') {
        ...
        $status = $data->createFunction($_POST['formFunction'],
