...
function doCreateLike($confirm, $msg = '')
{
    ...
    if (!$confirm) {
        ...
        foreach ($tbltmp as $a) {
            ...
            $tables["\"{$a['nspname']}\".\"{$a['relname']}\""] = serialize(array(
                'schema' => $a['nspname'],
                'table' => $a['relname']
            ));
        ...
        $status = $data->createTableLike($_REQUEST['name'], unserialize($_REQUEST['like']),
        ...
        if ($attrs->recordCount() > 0) {
            ...
            while (!$attrs->EOF) {
                ...
                if (($fksprops !== false) && isset($fksprops['byfield'][$attrs->fields['attnum']])) {
                    echo $data->printField("values[{$attrs->fields['attnum']}]",
                        $_REQUEST['values'][$attrs->fields['attnum']], 'fktype'/*force FK*/,
                        array(
                    ...
                    echo $data->printField("values[{$attrs->fields['attnum']}]",
                        $_REQUEST['values'][$attrs->fields['attnum']], $attrs->fields['type']);
                }
                echo "</td>\n";
                echo "</tr>\n";
                $i++;
        ...
        if (is_array($_REQUEST['table'])) {
            ...
            foreach ($_REQUEST['table'] as $t) {
                ...
                if ($status == 0) {
                    ...
                    doDefault(sprintf('%s%s: %s<br />', $msg, htmlentities($t, ENT_QUOTES, 'UTF-8'),
        ...
        if (is_array($_REQUEST['table'])) {
            ...
            if ($status == 0) {
                foreach ($_REQUEST['table'] as $t) {
                    ...
                    if ($status == 0) {
                        $msg .= sprintf('%s: %s<br />', htmlentities($t, ENT_QUOTES, 'UTF-8'),
