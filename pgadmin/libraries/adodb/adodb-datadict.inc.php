...
class ADODB_DataDict
{
    ...
    */
    function CreateIndexSQL($idxname, $tabname, $flds, $idxoptions = false)
    {
        ...
        return $this->_IndexSQL($this->NameQuote($idxname), $this->TableName($tabname), $flds,
    ...
    function RenameColumnSQL($tabname, $oldcolumn, $newcolumn, $flds = '')
    {
        ...
        return array(
            sprintf($this->renameColumn, $tabname, $this->NameQuote($oldcolumn), $this->NameQuote($newcolumn),
                $column_def)
        );
    }
        ...
        foreach ($lines as $id => $v) {
            if (isset($cols[$id]) && is_object($cols[$id])) {
                ...
                if ($flds && in_array(strtoupper(substr($flds[0][1], 0, 4)), $this->invalidResizeTypes4)
                    ...
                    if ($this->debug) {
                        ADOConnection::outp(sprintf("<h3>%s cannot be changed to %s currently</h3>", $flds[0][0],
                            $flds[0][1]));
                    }
