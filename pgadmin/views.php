    ...
    if ($confirm) {
        ...
        if ($attrs->recordCount() > 0) {
            ...
            while (!$attrs->EOF) {
                ...
                echo "<td style=\"white-space:nowrap;\">", $misc->printVal($data->formatType($attrs->fields['type'],
                    $attrs->fields['atttypmod'])), "</td>";
                echo "<td style=\"white-space:nowrap;\">";
            ...
            if ($status == 0) {
                foreach ($_POST['view'] as $s) {
                    ...
                    if ($status == 0) {
                        ...
                        $data->endTransaction();
                        doDefault(sprintf('%s%s: %s<br />', $msg, htmlentities($s, ENT_QUOTES, 'UTF-8'),
...
function doSetParamsCreate($msg = '')
{
    ...
    if (!isset($_POST['formTables'])) {
        ...
        for ($i = 0; $i < $linkCount; $i++) {
            ...
            if (!$rsLinkKeys->EOF) {
                $curLeftLink = htmlspecialchars(serialize(array(
                    'schemaname' => $rsLinkKeys->fields['p_schema'],
                    'tablename' => $rsLinkKeys->fields['p_table'],
                    'fieldname' => $rsLinkKeys->fields['p_field']
                )));
                $curRightLink = htmlspecialchars(serialize(array(
                    'schemaname' => $rsLinkKeys->fields['f_schema'],
                    'tablename' => $rsLinkKeys->fields['f_table'],
                    'fieldname' => $rsLinkKeys->fields['f_field']
                )));
                $rsLinkKeys->moveNext();
            } else {
                $curLeftLink = '';
            ...
            foreach ($_POST['formFields'] as $curField) {
                ...
                if (!empty($_POST['dblFldMeth'])) { // doublon control
                    if (empty($tmpHsh[$arrTmp['fieldname']])) { // field does not exist
                        ...
                        if ($_POST['dblFldMeth'] == 'rename') { // field exist and must be renamed
                ...
                if ($count > 0) {
                    ...
                    while ($j < $count) {
                        foreach ($arrLinks as $curLink) {
                            ...
                            if ((!in_array($curLink, $arrJoined) && in_array($tbl1,
                                        $arrUsedTbls)) || !count($arrJoined)) {
