    ...
    if ($confirm) {
        ...
        if ($rs->recordCount() == 1 && $attrs->recordCount() > 0) {
            ...
            while (!$attrs->EOF) {
                ...
                echo $data->printField("values[{$attrs->fields['attname']}]", $rs->fields[$attrs->fields['attname']],
                    $attrs->fields['type'], $extras);
        ...
        if (($k === $data->id) && (!($withOid && $conf['show_oids']))) {
            ...
            if (($v !== null) && isset($fkey_information['byfield'][$k])) {
                foreach ($fkey_information['byfield'][$k] as $conid) {
                    ...
                    echo "<a class=\"fk fk_" . htmlentities($conid, ENT_QUOTES,
                            'UTF-8') . "\" href=\"display.php?{$query_params}\">";
                ...
                echo $misc->printVal($v, $finfo->type,
                    array('null' => true, 'clip' => ($_REQUEST['strings'] == 'collapsed'), 'class' => 'fk_value'));
            } else {
                echo $misc->printVal($v, $finfo->type,
                    array('null' => true, 'clip' => ($_REQUEST['strings'] == 'collapsed')));
            }
            echo "</td>";
        }
    }
