...
function doDropMember($confirm)
{
    ...
    if ($confirm) {
        ...
        echo "<p>", sprintf($lang['strconfdropmember'], $misc->printVal($_REQUEST['user']),
            $misc->printVal($_REQUEST['group'])), "</p>\n";
...
function doCreate($msg = '')
{
    ...
    if ($users->recordCount() > 0) {
        ...
        while (!$users->EOF) {
            ...
            (in_array($username,
                $_POST['members']) ? ' selected="selected"' : ''), ">", $misc->printVal($username), "</option>\n";
            $users->moveNext();
        }
        echo "\t\t\t</select>\n";
        echo "\t\t</td>\n\t</tr>\n";
