...
class VLibTemplate
{
        ...
        if (is_array($arr) && !empty($arr)) {
            ...
            for ($i = 0; $i < count($arr); $i++) {
                ...
                foreach ($arr[$i] as $k => $v) {
                    ...
                    if (is_array($v)) {
                        if (($arr[$i][$k] = $this->_ArrayBuild($v)) === false) {
                            return false;
                        }
                    } else {
                        // reinsert the var
    ...
    function _ParseVar($wholeTag, $tag, $varName, $escape, $format, $nameSpace)
    {
        ...
        switch (strtolower($this->options['UNKNOWNS'])) {
            case 'comment':
                ...
                break;
            case 'leave':
                $retStr .= ' else { print("' . addcslashes($wholeTag,
                        '"') . '"); $this->_SetUnknown("' . $varName . '"); }';
                return $retStr;
                break;
            case 'print':
                $retStr .= ' else { print("' . htmlspecialchars($wholeTag,
                        ENT_QUOTES) . '"); $this->_SetUnknown("' . $varName . '"); }';
                return $retStr;
                break;
            case 'ignore':
                return $retStr;
    ...
    function _ParseTag($args)
    {
        ...
        if (preg_match("/^<\/|{\/|<!--\/$/s", $openclose) || preg_match("/^end[if|loop|unless|comment]$/", $tag)) {
            if ($tag == 'loop' || $tag == 'endloop') {
                array_pop($this->_nameSpace);
            }
        ...
        switch ($tag) {
            case 'var':
                $session = &GetSession();
                if ($session->GetProperty("EditVariables")) {
                    $escape = 'none';
                } elseif (empty($escape) && (!empty($this->options['DEFAULT_ESCAPE']) && strtolower($this->options['DEFAULT_ESCAPE']) != 'none')) {
            ...
            default:
                if ($this->options['STRICT']) {
                    VLibTemplateError::RaiseError('VT_ERROR_INVALID_TAG', KILL,
                        htmlspecialchars($wholeTag, ENT_QUOTES));
                }
                break;
        }
    }
