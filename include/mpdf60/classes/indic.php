...
// syllable_type
            ...
            // From OT spec:
                ...
                // From OT spec:
                    ...
                    if (($ptr == 0 ||
                        ...
                        && (preg_match('/^(RH|r)?[sD][N]?([ZJ]?H[CR]m*)?([M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr),
                            $ma))) {
                        // From HarfBuzz:
    ...
    public static function initial_reordering(
        &$info,
        $GSUBdata,
        $broken_syllables,
        $indic_config,
        ...
        for ($i = 0; $i < $count; $i++) {
            if ($info[$i]['indic_position'] == self::POS_BASE_C) {
                ...
                if (isset($GSUBdata['pref'][$c])) {
                    ...
                    if (isset($GSUBdata['blwf'][$c])) {
                        $info[$i]['indic_position'] = self::POS_BELOW_C;
                    } else {
                        if (isset($GSUBdata['pstf'][$c])) {
                            $info[$i]['indic_position'] = self::POS_POST_C;
                        }
                    }
                }
 ...
 * https://www.microsoft.com/typography/otfntdev/devanot/shaping.aspx */
    ...
    public static function initial_reordering_syllable(
        &$info,
        $GSUBdata,
        $indic_config,
        $scriptblock,
        ...
        for ($i = $start; $i < $end; $i++) {
            if ((self::FLAG($info[$i]['indic_category']) & (self::FLAG(self::OT_ZWJ) | self::FLAG(self::OT_ZWNJ) | self::FLAG(self::OT_N) | self::FLAG(self::OT_RS) | self::FLAG(self::OT_H) | self::FLAG(self::OT_Coeng)))) {
				...
				*/
                ...
                if ($info[$i]['indic_position'] != self::POS_SMVD) {
...
*/
    ...
    public static function final_reordering_syllable(
        &$info,
        $GSUBdata,
        $indic_config,
        $scriptblock,
