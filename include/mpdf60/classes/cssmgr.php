...
class cssmgr
{
    ...
    function ReadCSS($html)
    {
        ...
        for ($i = 0; $i < count($m[0]); $i++) {
            if ($this->mpdf->CSSselectMedia && !preg_match('/(' . trim($this->mpdf->CSSselectMedia) . '|all)/i',
                    $m[1][$i])) {
                ...
                if ($xem) {
                    for ($i = 0; $i < count($cxtem[0]); $i++) {
                        ...
                        if (!preg_match('/^data:image/i', $embedded)) {    // mPDF 5.5.13
                            ...
                            $CSSextblock = preg_replace('/' . preg_quote($cxtem[0][$i], '/') . '/',
                                ($cxtem[1][$i] . $embedded . $cxtem[3][$i]), $CSSextblock);
                        }
                    }
                }
                $CSSstr .= ' ' . $CSSextblock;
            ...
            if ($xem) {
                for ($i = 0; $i < count($cxtem[0]); $i++) {
                    ...
                    if (!preg_match('/^data:image/i', $embedded)) {    // mPDF 5.5.13
                        ...
                        $tmpCSSstr = preg_replace('/' . preg_quote($cxtem[0][$i], '/') . '/',
                            ($cxtem[1][$i] . $embedded . $cxtem[3][$i]), $tmpCSSstr);
                    }
                }
            }
            $CSSstr .= ' ' . $tmpCSSstr;
        ...
        if ($CSSstr) {
            ...
            for ($i = 0; $i < count($styles[1]); $i++) {
                ...
                foreach ($stylearr as $sta) {
                    if (trim($sta)) {
                        ...
                        if ($property && ($value || $value === '0')) {
                            ...
                            if ((strtoupper($property) == 'BACKGROUND-IMAGE' || strtoupper($property) == 'BACKGROUND') && preg_match('/-webkit-gradient/i',
                                    $value)) {
                                continue;
                            }
                ...
                foreach ($tagarr as $tg) {
                    // mPDF 5.7.4
                    if (preg_match('/NTH-CHILD\((\s*(([\-+]?\d*)N(\s*[\-+]\s*\d+)?|[\-+]?\d+|ODD|EVEN)\s*)\)/', $tg,
                        $m)) {
                            ...
                            if ($t) {
                                ...
                                if (preg_match('/^[.](.*)$/', $t, $m)) {
                                    ...
                                    if (preg_match('/^[#](.*)$/', $t, $m)) {
                                        ...
                                        if (preg_match('/^\[LANG=[\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\]$/', $t, $m)) {
                                            $tag = 'LANG>>' . strtolower($m[1]);
                                        }    // mPDF 6  Special case for lang as attribute selector
                                        else {
                                            if (preg_match('/^:LANG\([\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\)$/', $t,
                                                $m)) {
                                                $tag = 'LANG>>' . strtolower($m[1]);
                                            }    // mPDF 6  Special case for lang as attribute selector
                                            else {
                                                if (preg_match('/^(' . $this->mpdf->allowedCSStags . ')[.](.*)$/', $t,
                                                    ...
                                                    if (preg_match('/^(' . $this->mpdf->allowedCSStags . ')\s*:NTH-CHILD\((.*)\)$/',
                                                        $t, $m)) {
                                                        $tag = $m[1] . '>>SELECTORNTHCHILD>>' . $m[2];
                                                    } else {
                                                        if (preg_match('/^(' . $this->mpdf->allowedCSStags . ')[#](.*)$/',
                                                            $t, $m)) {
                                                            $tag = $m[1] . '>>ID>>' . $m[2];
                                                        } else {
                                                            if (preg_match('/^(' . $this->mpdf->allowedCSStags . ')\[LANG=[\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\]$/',
                                                                $t, $m)) {
                                ...
                                if (isset($this->CSS[$tag]) && $tag) {
                                    $this->CSS[$tag] = $this->array_merge_recursive_unique($this->CSS[$tag],
                                        $classproperties);
                                } else {
                                    if ($tag) {
                                        $this->CSS[$tag] = $classproperties;
...
//print_r($this->cascadeCSS); exit;
    ...
    function readInlineCSS($html)
    {
        ...
        for ($i = 0; $i < count($properties); $i++) {
            // Ignores -webkit-gradient so doesn't override -moz-
            if ((strtoupper($properties[$i]) == 'BACKGROUND-IMAGE' || strtoupper($properties[$i]) == 'BACKGROUND') && preg_match('/-webkit-gradient/i',
                    $values[$i])) {
                continue;
            }
                ...
                if (count($prop) == 3) {
                    ...
                    if (substr($prop[0], 0, 1) == '#') {
                        ...
                        if (substr($prop[0], 1, 1) == '#') {
                            ...
                            if (in_array($prop[0],
                                    $this->mpdf->borderstyles) || $prop[0] == 'none' || $prop[0] == 'hidden') {
                                $s = $prop[0];
                                $w = $prop[1];
                                $c = $prop[2];
                            } else {
                ...
                if ($k == 'FONT-FAMILY') {
                    ...
                    foreach ($aux_fontlist as $f) {
                        ...
                        if ((!$this->mpdf->onlyCoreFonts && in_array($v, $this->mpdf->available_unifonts)) ||
                            ...
                            ($this->mpdf->onlyCoreFonts && in_array($v,
                                    array('courier', 'times', 'helvetica', 'arial'))) ||
                    ...
                    if (!$found) {
                        foreach ($aux_fontlist as $f) {
                            ...
                            if (in_array($v, $this->mpdf->sans_fonts) || in_array($v,
                                    $this->mpdf->serif_fonts) || in_array($v, $this->mpdf->mono_fonts)) {
                                $newprop[$k] = $v;
                                break;
                            }
                        }
                    ...
                    if ($k == 'FONT-VARIANT') {
                        if (preg_match('/(normal|none)/', $v, $m)) {    // mPDF 6
                            ...
                            if (preg_match_all('/(no-common-ligatures|\bcommon-ligatures|no-discretionary-ligatures|\bdiscretionary-ligatures|no-historical-ligatures|\bhistorical-ligatures|no-contextual|\bcontextual)/i',
                                $v, $m)) {
                                $newprop['FONT-VARIANT-LIGATURES'] = implode(' ', $m[1]);
                            }
                            if (preg_match('/(all-small-caps|\bsmall-caps|all-petite-caps|\bpetite-caps|unicase|titling-caps)/i',
                                $v, $m)) {
                                $newprop['FONT-VARIANT-CAPS'] = $m[1];
                            }
                            if (preg_match_all('/(lining-nums|oldstyle-nums|proportional-nums|tabular-nums|diagonal-fractions|stacked-fractions)/i',
                                $v, $m)) {
                                                                ...
                                                                if ($k == 'BORDER-COLOR') {
                                                                    ...
                                                                    if ($k == 'BORDER-SPACING') {
                                                                        ...
                                                                        if ($k == 'TEXT-OUTLINE') {    // mPDF 5.6.07
                                                                            ...
                                                                            if ($k == 'SIZE') {
                                                                                $prop = preg_split('/\s+/', trim($v));
                                                                                if (preg_match('/(auto|portrait|landscape)/',
                                                                                    $prop[0])) {
                                                                                ...
                                                                                if ($k == 'SHEET-SIZE') {
                                                                                    $prop = preg_split('/\s+/',
                                                                                        trim($v));
                                                                                    if (count($prop) == 2) {
                                                                                        $newprop['SHEET-SIZE'] = array(
                                                                                            $this->mpdf->ConvertSize($prop[0]),
                                                                                            $this->mpdf->ConvertSize($prop[1])
                                                                                        );
                                                                                    } else {
                                                                                        if (preg_match('/([0-9a-zA-Z]*)-L/i',
                                                                                            $v,
                                                                                            $m)) {    // e.g. A4-L = A$ landscape
                                                                                    ...
                                                                                    if ($k == 'BACKGROUND') {
                                                                                        ...
                                                                                        if ($k == 'BACKGROUND-IMAGE') {
                                                                                            if (preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient\(.*\)/i',
                                                                                                $v, $m)) {
                                                                                                $newprop['BACKGROUND-IMAGE'] = $m[0];
                                                                                                continue;
                                                                                            }
                                                                                            if (preg_match('/url\([\'\"]{0,1}(.*?)[\'\"]{0,1}\)/i',
                                                                                                $v, $m)) {
                                                                                                ...
                                                                                                if ($k == 'BACKGROUND-POSITION') {
                                                                                                    ...
                                                                                                    if (count($bits) == 1) {
                                                                                                        ...
                                                                                                        if (count($bits) == 2) {
                                                                                                            ...
                                                                                                            if (preg_match('/(top|bottom)/',
                                                                                                                    $bits[0]) || preg_match('/(left|right)/',
                                                                                                                    $bits[1])) {
                                                                                                                $bg['p'] = $bits[1] . ' ' . $bits[0];
                                                                                                            } else {
                                                                                                                $bg['p'] = $bits[0] . ' ' . $bits[1];
                                                                                                    ...
                                                                                                    if ($bg['p']) {
                                                                                                        $bg['p'] = preg_replace('/(left|top)/',
                                                                                                            '0%',
                                                                                                            $bg['p']);
                                                                                                        $bg['p'] = preg_replace('/(right|bottom)/',
                                                                                                            '100%',
                                                                                                            $bg['p']);
                                                                                                        $bg['p'] = preg_replace('/(center)/',
                                                                                                            '50%',
                                                                                                            $bg['p']);
                                                                                                        if (!preg_match('/[\-]{0,1}\d+(in|cm|mm|pt|pc|em|ex|px|%)* [\-]{0,1}\d+(in|cm|mm|pt|pc|em|ex|px|%)*/',
                                                                                                            $bg['p'])) {
                                                                                                            $bg['p'] = false;
                                                                                                        }
                                                                                                    }
                                                                                                    ...
                                                                                                    if ($k == 'IMAGE-ORIENTATION') {
                                                                                                        if (preg_match('/([\-]*[0-9\.]+)(deg|grad|rad)/i',
                                                                                                            $v, $m)) {
                                                                                                            $angle = $m[1] + 0;
                                                                                                        ...
                                                                                                        if ($k == 'TEXT-ALIGN') {
                                                                                                            if (preg_match('/["\'](.){1}["\']/i',
                                                                                                                ...
                                                                                                                $m)) {
                                                                                                                $d = array_search($m[1],
                                                                                                                    $this->mpdf->decimal_align);
                                                                                                                if ($d !== false) {
                                                                                                                    $newprop['TEXT-ALIGN'] = $d;
                                                                                                                }
                                                                                                                if (preg_match('/(center|left|right)/i',
                                                                                                                    $v,
                                                                                                                    $m)) {
                                                                                                                    $newprop['TEXT-ALIGN'] .= strtoupper(substr($m[1],
                                                                                                                        0,
                                                                                                                        1));
                                                                                                                } else {
                                                                                                                ...
                                                                                                                if (preg_match('/["\'](\\\[a-fA-F0-9]{1,6})["\']/i',
                                                                                                                    $v,
                                                                                                                    $m)) {
                                                                                                                    $utf8 = codeHex2utf(substr($m[1],
                                                                                                                        1,
                                                                                                                        6));
                                                                                                                    $d = array_search($utf8,
                                                                                                                        $this->mpdf->decimal_align);
                                                                                                            ...
                                                                                                            if ($k == 'LIST-STYLE') {
                                                                                                                if (preg_match('/none/i',
                                                                                                                    $v,
                                                                                                                    $m)) {
                                                                                                                    $newprop['LIST-STYLE-TYPE'] = 'none';
                                                                                                                    $newprop['LIST-STYLE-IMAGE'] = 'none';
                                                                                                                }
                                                                                                                if (preg_match('/(lower-roman|upper-roman|lower-latin|lower-alpha|upper-latin|upper-alpha|decimal|disc|circle|square|arabic-indic|bengali|devanagari|gujarati|gurmukhi|kannada|malayalam|oriya|persian|tamil|telugu|thai|urdu|cambodian|khmer|lao|cjk-decimal|hebrew)/i',
                                                                                                                    $v,
                                                                                                                    $m)) {
                                                                                                                    $newprop['LIST-STYLE-TYPE'] = strtolower(trim($m[1]));
                                                                                                                } else {
                                                                                                                    if (preg_match('/U\+([a-fA-F0-9]+)/i',
                                                                                                                        $v,
                                                                                                                        $m)) {
                                                                                                                        $newprop['LIST-STYLE-TYPE'] = strtolower(trim($m[1]));
                                                                                                                    }
                                                                                                                ...
                                                                                                                if (preg_match('/(inside|outside)/i',
                                                                                                                    $v,
                                                                                                                    $m)) {
    ...
    {
        $sh = array();
        $c = preg_match_all('/(rgba|rgb|device-cmyka|cmyka|device-cmyk|cmyk|hsla|hsl)\(.*?\)/', $v,
            $x);    // mPDF 5.6.05
        for ($i = 0; $i < $c; $i++) {
        ...
        foreach ($ss as $s) {
            ...
            $p = explode(' ', trim($s));
            if (isset($p[0])) {
                $new['x'] = $this->mpdf->ConvertSize(trim($p[0]),
                    $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
            }
            if (isset($p[1])) {
                $new['y'] = $this->mpdf->ConvertSize(trim($p[1]),
                    $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
            }
            if (isset($p[2])) {
                if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
                    $new['blur'] = $this->mpdf->ConvertSize(trim($p[2]),
                        $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
                } else {
                ...
                if (isset($p[3])) {
                    if (preg_match('/^\s*[\.\-0-9]/', $p[3])) {
                        $new['spread'] = $this->mpdf->ConvertSize(trim($p[3]),
                            $this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'], $this->mpdf->FontSize, false);
                    } else {
    ...
    function setCSStextshadow($v)
    {
        ...
        foreach ($ss as $s) {
            ...
            if (isset($p[2])) {
                if (preg_match('/^\s*[\.\-0-9]/', $p[2])) {
                    $new['blur'] = $this->mpdf->ConvertSize(trim($p[2]),
                        $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->FontSize, false);
                } else {
                    $new['col'] = $this->mpdf->ConvertColor(preg_replace('/\*/', ',', $p[2]));
                }
                if (isset($p[3])) {
    ...
    function parseCSSbackground($s)
    {
        ...
        if (preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient\(.*\)/i', $s, $m)) {
            ...
            if (preg_match('/url\(/i', $s)) {
                ...
                // mPDF 5.6.05
                if (preg_match('/^\s*(#[0-9a-fA-F]{3,6}|(rgba|rgb|device-cmyka|cmyka|device-cmyk|cmyk|hsla|hsl|spot)\(.*?\)|[a-zA-Z]{3,})\s+(url\(.*)/i',
                    $s, $m)) {
                    $bg['c'] = strtolower($m[1]);
                    $s = $m[3];
                }
                ...
                if (preg_match('/^\s*(#[0-9a-fA-F]{3,6}|(rgba|rgb|device-cmyka|cmyka|device-cmyk|cmyk|hsla|hsl|spot)\(.*?\)|[a-zA-Z]{3,})/i',
                    $s, $m)) {
                    $bg['c'] = strtolower($m[1]);
                }
            }
...
// for CSS handling
            ...
            foreach ($p as $k => $val) {
                if (preg_match('/' . $tag . '>>SELECTORNTHCHILD>>(.*)/', $k, $m)) {
                    ...
                    if ($tag == 'TR') {
                        ...
                        if ($tag == 'TD' || $tag == 'TH') {
                            if (preg_match('/(([\-+]?\d*)?N([\-+]\d+)?|[\-+]?\d+|ODD|EVEN)/', $m[1],
                                $a)) {    // mPDF 5.7.4
                ...
                if (isset($a['BORDER-' . $side . '-' . $el])) {    // e.g. $b['BORDER-TOP-STYLE']
                    ...
                    if ($el == 'STYLE') {
                        ...
                        if ($el == 'WIDTH') {
                            ...
                            if ($el == 'COLOR') {
                                if ($p) {
                                    $b['BORDER-' . $side] = preg_replace('/(\S+)\s+(\S+)\s+(\S+)/', '\\1 \\2 ' . $s,
                                        $p);
                                } else {
    ...
    function MergeCSS($inherit, $tag, $attr)
    {
        ...
        if ($inherit == 'BLOCK') {
            ...
            //===============================================
            // Save Cascading CSS e.g. "div.topic p" at this block level
            $this->_mergeFullCSS($this->cascadeCSS, $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes,
                $attr['ID'], $attr['LANG']);
            //===============================================
            // Cascading forward CSS
            //===============================================
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1])) {
                $this->_mergeFullCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS'],
                    $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, $attr['ID'], $attr['LANG']);
            }
            //===============================================
            ...
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) && $this->mpdf->blk[$this->mpdf->blklvl - 1]['align']) {
                if ($this->mpdf->blk[$this->mpdf->blklvl - 1]['align'] == 'L') {
                    ...
                    if ($this->mpdf->blk[$this->mpdf->blklvl - 1]['align'] == 'J') {
                        $p['TEXT-ALIGN'] = 'justify';
                    } else {
                        if ($this->mpdf->blk[$this->mpdf->blklvl - 1]['align'] == 'R') {
                            $p['TEXT-ALIGN'] = 'right';
                        } else {
                            if ($this->mpdf->blk[$this->mpdf->blklvl - 1]['align'] == 'C') {
                                $p['TEXT-ALIGN'] = 'center';
                            }
                        }
                    }
        ...
        if (isset($attr['LANG'])) {
            if (isset($this->CSS['LANG>>' . $attr['LANG']]) && $this->CSS['LANG>>' . $attr['LANG']]) {
                ...
                if (isset($this->CSS['LANG>>' . $shortlang]) && $this->CSS['LANG>>' . $shortlang]) {
        ...
        if (isset($attr['LANG'])) {
            if (isset($this->CSS[$tag . '>>LANG>>' . $attr['LANG']]) && $this->CSS[$tag . '>>LANG>>' . $attr['LANG']]) {
                ...
                if (isset($this->CSS[$tag . '>>LANG>>' . $shortlang]) && $this->CSS[$tag . '>>LANG>>' . $shortlang]) {
        ...
        if ($inherit == 'BLOCK') {
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1])) {    // mPDF 6
                ...
                foreach ($classes as $class) {
                    $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS']['CLASS>>' . $class],
                        $p);
                }
                $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS']['ID>>' . $attr['ID']],
                    $p);
                foreach ($classes as $class) {
                    $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS'][$tag . '>>CLASS>>' . $class],
                        $p);
                }
                $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl - 1]['cascadeCSS'][$tag . '>>ID>>' . $attr['ID']],
                    $p);
            }
        } else {
            if ($inherit == 'INLINE') {
                ...
                foreach ($classes as $class) {
                    $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'][$tag . '>>CLASS>>' . $class],
                        $p);
                }
                $this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'][$tag . '>>ID>>' . $attr['ID']],
                    $p);
            } /*-- TABLES --*/
            else {
                if ($inherit == 'TOPTABLE' || $inherit == 'TABLE') { // NB looks at $this->tablecascadeCSS-1 for cascading CSS
                    if (isset($this->tablecascadeCSS[$this->tbCSSlvl - 1])) {    // mPDF 6
                        ...
                        foreach ($classes as $class) {
                            $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1]['CLASS>>' . $class], $p,
                                false, 9);
                        }
                        // STYLESHEET nth-child SELECTOR e.g. tr:nth-child(odd)  td:nth-child(2n+1)
                        if ($tag == 'TR' || $tag == 'TD' || $tag == 'TH') {
                            foreach ($this->tablecascadeCSS[$this->tbCSSlvl - 1] as $k => $val) {
                                if (preg_match('/' . $tag . '>>SELECTORNTHCHILD>>(.*)/', $k, $m)) {
                                    ...
                                    if ($select) {
                                        $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>SELECTORNTHCHILD>>' . $m[1]],
                                            $p, false, 9);
                                    }
                                }
                            }
                    ...
                    $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1]['ID>>' . $attr['ID']], $p, false,
                        9);
                    foreach ($classes as $class) {
                        $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>CLASS>>' . $class],
                            $p, false, 9);
                    }
                    $this->_set_mergedCSS($this->tablecascadeCSS[$this->tbCSSlvl - 1][$tag . '>>ID>>' . $attr['ID']],
                        $p, false, 9);
                }
            }
        }
    ...
    function inlinePropsToCSS($bilp, &$p)
    {
        ...
        if (isset($bilp['textvar']) && $bilp['textvar']) {
            ...
            if ($bilp['textvar'] & FT_CAPITALIZE) {
                ...
                if ($bilp['textvar'] & FT_UPPERCASE) {
                    $p['TEXT-TRANSFORM'] = 'uppercase';
                } else {
                    if ($bilp['textvar'] & FT_LOWERCASE) {
