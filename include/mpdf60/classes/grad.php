...
// mPDF 5.3.A1
    function CoonsPatchMesh(
        ...
        $s = ' q ';
        $s .= sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * _MPDFK, ($this->mpdf->h - $y) * _MPDFK, $w * _MPDFK,
            -$h * _MPDFK);
        $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * _MPDFK, $h * _MPDFK, $x * _MPDFK,
            ($this->mpdf->h - ($y + $h)) * _MPDFK);
...
// $stops = array('col'=>$col [, 'opacity'=>0-1] [, 'offset'=>0-1])
    function Gradient(
        ...
        if (strtoupper(substr($type, 0, 1)) == 'L') {
            ...
            if (strtoupper(substr($type, 0, 1)) == 'R') {
                $type = 3;
            }
        }    // radial
        ...
        if ($type == 2) {
            ...
            // NO end points (Angle defined & Start points)
                ...
                if ($angle !== false && $coords[0] !== false && $coords[1] !== false && $coords[2] === false && $coords[3] === false) {
                ...
                // NO end points or Start points (Angle defined)
                    ...
                    if ($angle !== false && $coords[0] === false && $coords[1] === false) {
                        if ($angle == 0 || $angle == 360) {
                            $coords[0] = 0;
                            $coords[1] = 0;
                            $coords[2] = 1;
                    ...
                    // NO angle and NO end points (Start points defined)
                        ...
                        if ((!isset($angle) || $angle === false) && $coords[0] !== false && $coords[1] !== false) {    // should have start and end defined
                            $coords[2] = 1 - $coords[0];
                            $coords[3] = 1 - $coords[1];
            ...
            $s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x * _MPDFK, ($this->mpdf->h - $y) * _MPDFK, $w * _MPDFK,
                    -$h * _MPDFK) . "\n";
            $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * _MPDFK, $useh * _MPDFK, $usex * _MPDFK,
                    ($this->mpdf->h - ($usey + $useh)) * _MPDFK) . "\n";
        } // RADIAL
        else {
            if ($type == 3) {
                        ...
                        if ($shape == 'ellipse') {
                            ...
                            if ($shape == 'circle') {
                                if ($w >= $h) {
                                    ...
                                    if ($size == 'closest-side') {
                                        $radius = min($coords[0], $coords[1], (1 - $coords[0]),
                                            (($h / $w) - $coords[1]));
                                    } else {
                                        if ($size == 'closest-corner') {
                                            $radius = min($corner1, $corner2, $corner3, $corner4);
                                            ...
                                            if ($size == 'farthest-side') {
                                                $radius = max($coords[0], $coords[1], (1 - $coords[0]),
                                                    (($h / $w) - $coords[1]));
                                            } else {
                                    ...
                                    if ($size == 'closest-side') {
                                        $radius = min($coords[0], $coords[1], (($w / $h) - $coords[0]),
                                            (1 - $coords[1]));
                                    } else {
                                        if ($size == 'closest-corner') {
                                            $radius = min($corner1, $corner2, $corner3, $corner4);
                                            ...
                                            if ($size == 'farthest-side') {
                                                $radius = max($coords[0], $coords[1], (($w / $h) - $coords[0]),
                                                    (1 - $coords[1]));
                                            } else {
                                                if ($size == 'farthest-corner') {
                                                    $radius = max($corner1, $corner2, $corner3, $corner4);
        ...
        for ($i = 0; $i < count($stops); $i++) {
            if (isset($stops[$i]['offset']) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $stops[$i]['offset'],
                    $m)) {
                $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
        ...
        for ($i = 0; $i < count($stops); $i++) {
            // mPDF 5.3.74
            if ($colorspace == 'CMYK') {
                $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F %.3F',
                    (ord($stops[$i]['col']{1}) / 100), (ord($stops[$i]['col']{2}) / 100),
                    (ord($stops[$i]['col']{3}) / 100), (ord($stops[$i]['col']{4}) / 100));
            } else {
                if ($colorspace == 'Gray') {
                    $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F', (ord($stops[$i]['col']{1}) / 255));
                } else {
                    $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F %.3F %.3F',
                        (ord($stops[$i]['col']{1}) / 255), (ord($stops[$i]['col']{2}) / 255),
                        (ord($stops[$i]['col']{3}) / 255));
                }
            }
            if (!isset($stops[$i]['opacity'])) {
                $stops[$i]['opacity'] = 1;
            } else {
                if ($stops[$i]['opacity'] > 1 || $stops[$i]['opacity'] < 0) {
                    $stops[$i]['opacity'] = 1;
                } else {
                    if ($stops[$i]['opacity'] < 1) {
                        $trans = true;
                    }
                }
            }
    ...
    function parseMozGradient($bg)
    {
        ...
        if (preg_match('/linear-gradient\((.*)\)/', $bg, $m)) {
            ...
            if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i',
                    $bgr[0]) && !preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0])) {
                $startStops = 1;
            } else {
                if (trim($first[(count($first) - 1)]) === "0") {
                    $startStops = 1;
                } else {
            ...
            if ($startStops == 1) {    // default values
                ...
                if (preg_match('/(\d+)[%]/i', $first[0], $m)) {
                    ...
                    if (!isset($startx) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[0], $m)) {
                        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
                        if ($tmp) {
                            $startx = $m[1];
                        }
                ...
                if (isset($first[1]) && preg_match('/(\d+)[%]/i', $first[1], $m)) {
                    ...
                    if (!isset($starty) && isset($first[1]) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i',
                            $first[1], $m)) {
                        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
                        if ($tmp) {
                            $starty = $m[1];
            ...
            for ($i = $startStops; $i < count($bgr); $i++) {
                ...
                if (isset($el[1]) && preg_match('/(\d+)[%]/', $el[1], $m)) {
                    ...
                    if (isset($el[1]) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $el[1], $m)) {
                        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
                        if ($tmp) {
                            $stop['offset'] = $m[1];
                        }
            ...
            if (preg_match('/radial-gradient\((.*)\)/', $bg, $m)) {
                $g = array();
                $g['type'] = 3;
                $g['colorspace'] = 'RGB';
                $g['extend'] = array('true', 'true');
                ...
                if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i',
                    ...
                    if (trim($first[(count($first) - 1)]) === "0") {
                        ...
                        if (preg_match('/(circle|ellipse|closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i',
                            $bgr[0])) {
                            $startStops = 1;
                            $shape_size = $bgr[0];
                        } else {
                ...
                if ($pos_angle) {    // default values
                    ...
                    if (preg_match('/left/i', $pos_angle)) {
                        ...
                        if (preg_match('/right/i', $pos_angle)) {
                            $startx = 1;
                        }
                    }
                    if (preg_match('/top/i', $pos_angle)) {
                        ...
                        if (preg_match('/bottom/i', $pos_angle)) {
                            $starty = 0;
                        }
                    }
                    // Check for %? ?% or %%
                ...
                if ($shape_size) {    // default values
                    ...
                    if (preg_match('/(closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i',
                        $shape_size, $m)) {
                        $size = $m[1];
                        if ($size == 'contain') {
                            $size = 'closest-side';
    ...
    function parseBackgroundGradient($bg)
    {
        ...
        if (count($bgr) > 6) {
            ...
            $g['extend'] = array('true', 'true');
            $g['stops'] = array(
                array('col' => $g['col'], 'opacity' => 1, 'offset' => 0),
                array('col' => $g['col2'], 'opacity' => 1, 'offset' => 1)
            );
            return $g;
        }
        return false;
