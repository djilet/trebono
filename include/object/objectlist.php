...
class ObjectList extends CommonObject
{
    ...
    function LoadFromSQL($query, $statement = null)
    {
        if (preg_match('/^\(?\s*select/is', $query)) {
            ...
            if ($this->GetItemsOnPage() > 0) {
                ...
                $q = $query . $this->GetOrderBySQLString() . " LIMIT " . $this->GetItemsOnPage() . " OFFSET " . $start;
                $items = $statement->FetchList($q);

                $q = "SELECT COUNT(*) FROM (" . $query . ") AS sub";
                $this->_SetCountTotalItems($statement->FetchField($q));
                ...
                $this->_SetCountPages(1);
                $this->_SetCountTotalItems(count($items));
            }
    ...
    function GetPagingAsArray($url, $urlFirstPage = null)
    {
        ...
        if (!isset($urlComponents["query"])) {
            ...
            if (substr($urlComponents["query"], 0, 1) != "?") {
    ...
    function GetItemsRange()
    {
        ...
        if ($last > $totalItems || $itemsOnPage == 0) {
            $last = $totalItems;
        }
