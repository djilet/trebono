...
class Session extends Object
{
    ...
    function SaveToDB($inCookie = null)
    {
        ...
        if (count($this->GetProperties()) > 0) {
            ...
            if ($this->UpdateCookie($sessionInterval)) {
                $stmt->Execute("INSERT INTO user_session
					(session_id, in_cookie, expire_date, session_data, user_id)
                ...
                $stmt->Execute("UPDATE user_session SET
					in_cookie=" . $inCookie . ",
					expire_date=NOW()+INTERVAL '" . $sessionInterval . " SECOND',
					session_data=" . Connection::GetSQLString(serialize($a)) . ",
					user_id=" . Connection::GetSQLString($userID) . "
					WHERE session_id=" . Connection::GetSQLString($this->_sessionID));
            }
            ...
            $stmt->Execute("DELETE FROM user_session WHERE
				session_id=" . Connection::GetSQLString($this->_sessionID));
        }
    }
    ...
    function UpdateCookie($cookieInterval)
    {
        ...
        // Update cookie only if its expiration time is changing more than for a hour
        $cookie = new Object($_COOKIE);
        $expirationKey = $this->_sessionName . "_expires";
        $expirationOld = $cookie->GetIntProperty($expirationKey);
        $expirationNew = $this->_inCookie ? time() + $cookieInterval : 0;

        if ($newSession == true || abs($expirationNew - $expirationOld) > 3600) {
            setcookie($this->_sessionName, $this->_sessionID, $expirationNew, PROJECT_PATH);
            setcookie($expirationKey, $expirationNew, $expirationNew, PROJECT_PATH);

            // Put expiration value directly to $_COOKIE to make it accessible by the next calls of UpdateCookie
            $_COOKIE[$expirationKey] = $expirationNew;
        }
    ...
    function LoadFromDB()
    {
        ...
        $query = "SELECT in_cookie, session_data FROM user_session
			WHERE session_id=" . Connection::GetSQLString($this->_sessionID);
        ...
        if ($data = $stmt->FetchRow($query)) {
            ...
            if ($a = unserialize($data["session_data"])) {
    ...
    {
        $stmt = GetStatement(DB_PERSONAL);
        $stmt->Execute("DELETE FROM user_session WHERE expire_date < NOW()");
    }
