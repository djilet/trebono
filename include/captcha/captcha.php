...
while (($file = readdir($dh)) !== false) {
    if (is_file($websiteDir . $file) && $file != "." && $file != "..") {
        ...
        if ($info) {
            ...
            if (strtolower(substr($file, -4, 4)) == '.ttf') {
...
if (count($textColors) == 0) {
    $textColors = array(imagecolorallocate($srcImg, 0, 0, 0));
}
srand((float)microtime() * 1000000);
shuffle($textColors);
...
if (count($ttfFonts) > 0) {
    ...
    for ($i = 0; $i < count($code); $i++) {
        ...
        imagettftext($srcImg, $size, $rotate, ($i + 1) * 27 - 5, 35, $textColors[mt_rand(0, count($textColors) - 1)],
            $ttfFonts[0], $code[$i]);
    }
} else {
