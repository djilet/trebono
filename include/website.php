...
class Website
{
    ...
    function Website()
    {
        ...
        // Trick for console launch (tests, phinx) support for single-website project
        if (empty($_SERVER["HTTP_HOST"]) && empty($_SERVER["REQUEST_URI"]) && count($this->_websiteList) == 1) {
            $firstWebsite = reset($this->_websiteList);
            $firstLanguage = reset($firstWebsite["Language"]);
            $firstDomain = reset($firstLanguage["Domain"]);

            $_SERVER["HTTP_HOST"] = $firstDomain["HostName"];
            $_SERVER["REQUEST_URI"] = $firstWebsite["WebDir"];
        }
        ...
        if (defined('IS_ADMIN')) {
            ...
            foreach ($this->_websiteList as $websiteID => $website) {
                ...
                $adminURL = !is_null($webHost)
                    ? GetFromConfig("ProtocolType") . $webHost . $website["WebDir"] . ADMIN_FOLDER . "/index.php"
                    : "";
        ...
        if (count($current) > 0) {
            ...
            foreach ($current['LanguageList'] as $k => $v) {
                ...
                $domainInfo = $v['Domain'][$host] ?? array_shift($v['Domain']);

                unset($v['Domain']);

                $domainInfo['LangURL'] = GetFromConfig("ProtocolType") . $domainInfo['HostName'] . "/" . $domainInfo['LangDir'];
    ...
    function CheckForRedirect($domainList, $currentDomain)
    {
        ...
        if (isset($hostData['Redirect'])) {
            if ($hostData['Redirect'] != 'no') {
                Send301(GetFromConfig("ProtocolType") . $hostData['Redirect'] . $_SERVER['REQUEST_URI']);
                exit;
            }
        }
        ...
        // if current domain is not first
            ...
            foreach ($domainList as $k => $v) {
                if ($k != $currentDomain) {
                    Send301(GetFromConfig("ProtocolType") . $k . $_SERVER['REQUEST_URI']);
                    exit;
                }
                break;
            }
    ...
    function _GetWebsiteXMLAsArray($file)
    {
        ...
        foreach ($this->_values as $id => $value) {
            ...
            if (
                isset($value["attributes"]["WebsiteID"]) &&
                ...
                if (
                    isset($value["attributes"]["ModuleList"]) &&
                ...
                if ($value["type"] == "close" && !is_null($websiteID)) {
                    $websites[$websiteID][$value["type"]] = $id;
                    $websiteID = null;
                }
            }
        ...
        foreach ($websites as $id => $details) {
            ...
            if (isset($details["open"]) && isset($details["close"])) {
                for ($i = $details["open"] + 1; $i < $details["close"]; $i++) {
                    if ($this->_values[$i]["level"] == 3 && $this->_values[$i]["tag"] == "Language" && ($this->_values[$i]["type"] == "open" || $this->_values[$i]["type"] == "complete")) {
                        for ($j = 0; $j < sizeof($requiredLangAttributes); $j++) {
                            ...
                            ErrorHandler::TriggerError(
                                "Attribute \"" . $requiredLangAttributes[$j] . "\" is absent for tag <Language> in file \"" . $file . "\"!",
                                E_ERROR
                            );
                        }
                        ...
                        if ($this->_values[$i]["level"] == 3 && $this->_values[$i]["tag"] == "Language" && $this->_values[$i]["type"] == "close") {
                            $langCode = null;
                        }
                    }
                    ...
                    for ($j = 0; $j < sizeof($requiredDomainAttributes); $j++) {
                        ...
                        ErrorHandler::TriggerError(
                            "Attribute \"" . $requiredDomainAttributes[$j] . "\" is absent for tag <Domain> in file \"" . $file . "\"!",
                            E_ERROR
                        );
                    }
            ...
            if (count($websites[$id]["Language"]) == 0) {
                ErrorHandler::TriggerError(
                    "No tag <Language> is defined under tag <Website> in the XML file \"" . $file . "\"",
                    E_ERROR
                );
            }
