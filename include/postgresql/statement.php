...
class Statement
{
    ...
    function Statement($dbLink, $resultType = PGSQL_ASSOC, $errorLevel = E_USER_WARNING)
    {
        $this->_dbLink = $dbLink;
    ...
    {
        $result = pg_query($this->_dbLink, $query);
        $retVal = null;
        $this->_affectedRows = null;
        $this->_lastInsertID = null;
        $this->_numRows = null;
        ...
        if ($status === PGSQL_COMMAND_OK) {
            ...
            if (strpos($query, "RETURNING")) {
                $ret = pg_fetch_row($result);
                if ($ret) {
                    $this->_lastInsertID = $ret[0];
                }
            }
            ...
            $this->_RaiseError($query, $status, pg_last_error($this->_dbLink));
            $retVal = false;
        }
    ...
    {
        $rs =& $this->Execute($query, PGSQL_BOTH);
        if (strtolower(get_class($rs)) == 'recordset') {
            $result = $rs->AllRows();
            $indexedResult = array();
    ...
    function FetchAssocIndexedList($query, $field = '')
    {
        $rs =& $this->Execute($query, PGSQL_ASSOC);
        if (strtolower(get_class($rs)) == 'recordset') {
            $results = $rs->AllRows();
            $indexedResult = array();
    ...
    {
        if ($this->_errorLevel) {
            ErrorHandler::TriggerError('(' . $errNo . ') ' . $errStr . ' in query: ' . $this->_ToLog($query),
                $this->_errorLevel);
        }
    }
}
