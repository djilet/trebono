...
class URLParser
{
    ...
    function GetFixedPathAsArray()
    {
        ...
        if (count($this->fixedPath) == 0) {
            ...
            if (count($this->fixedPath) == 1) {
                if ($this->fixedPath[0] == "") {
                    $this->fixedPath[0] = INDEX_PAGE;
                } else {
                    if (substr($this->fixedPath[0], -strlen($this->fileExtension)) == $this->fileExtension) {
                        $this->fixedPath[0] = substr(
                            $this->fixedPath[0],
                            0,
                            strlen($this->fixedPath[0]) - strlen($this->fileExtension)
                        );
                    }
                }
            } else {
                ...
                if ($this->fixedPath[$lastIndex] == INDEX_PAGE . HTML_EXTENSION || $this->fixedPath[$lastIndex] == "") {
                    ...
                    if (substr($this->fixedPath[$lastIndex], -strlen($this->fileExtension)) == $this->fileExtension) {
                        $this->fixedPath[$lastIndex] = substr(
                            $this->fixedPath[$lastIndex],
                            0,
                            strlen($this->fixedPath[$lastIndex]) - strlen($this->fileExtension)
                        );
                    }
                }
            }
        ...
        if (count($this->fixedPath) == 1 && $this->fixedPath[0] == INDEX_PAGE) {
            ...
            if ($this->fixedPath[count($this->fixedPath) - 1] == $this->shortPath[count($this->shortPath) - 1]) {
    ...
    function IsHTML()
    {
        ...
        return $this->contentType == 'text/html' && ($this->fileExtension == HTML_EXTENSION || $this->fileExtension == ".");
    }
}
