...
class ErrorHandler
{
    ...
    function ErrorHandler()
    {
        ...
        $this->saveErrors = (E_ALL | E_STRICT) ^ E_DEPRECATED; // if no need to save errors - set to false (false, null, 0)

        if (GetFromConfig('DevMode', 'common')) {
            $this->showErrors = (E_ALL | E_STRICT) ^ E_DEPRECATED;
        } else {
    ...
    /**
     * Invoke an error @param string $errStr Error message
     * @param int $errNo bit mask of errors default E_USER_ERROR
     *
     * @return void
     * @see trigger_error but can set any error level
     *
     */
    ...
    function _SaveLog()
    {
        ...
        if (count($errors) > 0) {
            ...
            if (strlen($errList) > 0) {
                ...
                $fileStorage->PutFileContent($this->pathToLogFile, $stringToWrite, true);
            }
        }
