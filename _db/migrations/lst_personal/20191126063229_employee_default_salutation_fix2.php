...
class EmployeeDefaultSalutationFix2 extends AbstractMigration
{
    public function change()
    {
        ...
        foreach ($userList as $user)
        {
            ...
            {
                $this->execute("UPDATE user_info SET salutation='Herr' WHERE user_id=".Connection::GetSQLString($user["user_id"]));
            ...
            {
                $this->execute("UPDATE user_info SET salutation='Frau' WHERE user_id=".Connection::GetSQLString($user["user_id"]));
                $query = "INSERT INTO user_history (end_user_id, property_name, value, created, start_user_id, created_from)
					VALUES (
					".$user["user_id"].",
					'salutation',
        ...
        foreach ($contactList as $contact)
        {
            ...
            {
                $this->execute("UPDATE contact SET salutation='Herr' WHERE contact_id=".Connection::GetSQLString($contact["contact_id"]));
            ...
            {
                $this->execute("UPDATE contact SET salutation='Frau' WHERE contact_id=".Connection::GetSQLString($contact["contact_id"]));
                $query = "INSERT INTO contact_history (contact_id, property_name, value, created, user_id, created_from)
                    VALUES (
                    ".$contact["contact_id"].",
                    'salutation',
        ...
        foreach ($contactList as $contact)
        {
            if ($contact["salutation"] === "" || $contact["salutation"] == "HErr" || $contact["salutation"] == "Herr" || $contact["salutation"] == "Herr" ||
                $contact["salutation"] == "Mr" || $contact["salutation"] == "Mr." || $contact["salutation"] == "mr")
            {
                $this->execute("UPDATE partner_contact SET salutation='Herr' WHERE partner_contact_id=".Connection::GetSQLString($contact["partner_contact_id"]));
            ...
            {
                $this->execute("UPDATE partner_contact SET salutation='Frau' WHERE partner_contact_id=".Connection::GetSQLString($contact["partner_contact_id"]));
                $query = "INSERT INTO partner_contact_history (contact_id, property_name, value, created, user_id)
					VALUES (
					".$contact["partner_contact_id"].",
					'salutation',
