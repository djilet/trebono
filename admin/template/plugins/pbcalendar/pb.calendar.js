jQuery(function($){
	...
	var _pb_calendar = (function(target_, options_){
		...
		this._target.append('<div class="calendar-head-frame">' +
			'<div class="row row-dayname">' +
				...
				'<div class="col col-dayname">DIE</div>' +
				'<div class="col col-dayname">MIT</div>' +
				'<div class="col col-dayname">DON</div>' +
				'<div class="col col-dayname">FRE</div>' +
				'<div class="col col-dayname holiday">SAM</div>' +
				'<div class="col col-dayname holiday">SON</div>' +
			'</div>' +
		'</div>');
	...
	_pb_calendar.prototype.yyyymm = (function(yyyymm_){
		if(yyyymm_ !== undefined){
			...
			//set german locale for month name
			moment.locale('de')
	...
	_pb_calendar.prototype.update_view = (function(){
		...
		this._schedule_list(function(schedule_list_){
			...
			$.each(schedule_list_, function(yyyymmdd_, schedule_data_){
				...
				var schedule_el_ = that_._body_frame.find("[data-day-yyyymmdd='"+yyyymmdd_+"'] > a.day-label");
				...
				$.each(schedule_data_, function(){
					...
					that_._options['schedule_dot_item_render'].apply(that_, [schedule_dot_item_, this, schedule_el_]);
				});
